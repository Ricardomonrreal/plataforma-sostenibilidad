    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm shadow-slate-200/40 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          />
          {pobladoApoyo === 'Si' && (
            <TextInputField label="8. ¿Cuántos colaboradores residen en el poblado de apoyo?" />
          )}
        </div>
      </div>
      ...
      <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 shadow-sm shadow-slate-200/40 space-y-6">
        ...
        <div className="flex flex-col gap-6">
          <TextInputField label="9. ¿Cuántos eventos internos para colaboradores se organizaron durante el mes (listar)?"/>
          <SelectInputField 
            label="10. ¿Se organizó alguna actividad de voluntariado durante el mes?" 
            options={["Si", "No"]} 
            placeholder="Seleccionar..." 
            onChange={(e) => setVoluntariado(e.target.value)} 
          />
          {voluntariado === 'Si' && (
            <TextInputField label="11. Si la respuesta a la pregunta anterior fue 'si', favor de indicar qué actividad(es) se organizaron y el número total de horas de voluntariado de cada una de ellas." subtitle="Para obtener el total de horas de voluntariado, se debe usar la siguiente fórmula: Duración de la actividad x total de colaboradores que participaron en ella."/>
          )}
          <TextInputField label="12. ¿A cuánto asciende el monto donado a Fundación Lomas durante el mes?" subtitle="Moneda nacional" placeholder='$0.00'/>
          <TextInputField label="13. Número de colaboradores en el programa de educación para adultos (programa interno o externo)." subtitle="Personas" placeholder='0'/>
          <TextInputField label="14. Monto en efectivo donado a causas benéficas (especificar Fundación). " subtitle="Moneda nacional (No considerar fundación)" placeholder='$0.00'/>
          <TextInputField label="15. Valor aproximado en dinero, de los donativos de blancos, activos fijos y otros donativos en especie." subtitle="Moneda nacional. Cálculo del monto que valdrían los donativos en especie, si estos hubieran sido vendidos (ej. Una sábana usada vale el 20% de su valor original)" placeholder='$0.00'/>
          <FileUploadField label="16. Favor de adjuntar la evidencia disponible de los eventos, donativos, campañas, calendario ambiental, capacitaciones y otras actividades del mes." subtitle="Puede ser el reporte mensual de evidencias de RRHH"/>
          <FileUploadField label="17. Adjunta el reporte de indicadores mensual" subtitle="Rotación, distribución geográfica y demográfica, horas capacitación, etc." />
        </div>
      </div>
